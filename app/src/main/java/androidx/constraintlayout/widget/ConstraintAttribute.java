        ...
        for (String str : hashMap.keySet()) {
            ...
            try {
                if (str.equals("BackgroundColor")) {
                    ...
                    hashMap2.put(str, new ConstraintAttribute(constraintAttribute, cls.getMethod(valueOf.length() != 0 ? "getMap".concat(valueOf) : new String("getMap"), new Class[0]).invoke(view, new Object[0])));
                }
    ...
    public void setInterpolatedValue(View view, float[] fArr) {
        ...
        try {
            switch (this.mType) {
                ...
                case COLOR_TYPE:
                    cls.getMethod(concat, Integer.TYPE).invoke(view, Integer.valueOf((clamp((int) (((float) Math.pow(fArr[0], 0.45454545454545453d)) * 255.0f)) << 16) | (clamp((int) (fArr[3] * 255.0f)) << 24) | (clamp((int) (((float) Math.pow(fArr[1], 0.45454545454545453d)) * 255.0f)) << 8) | clamp((int) (((float) Math.pow(fArr[2], 0.45454545454545453d)) * 255.0f))));
                    return;
                case COLOR_DRAWABLE_TYPE:
