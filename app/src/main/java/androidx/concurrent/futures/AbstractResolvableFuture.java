...
public abstract class AbstractResolvableFuture<V> implements bxl<V> {
    ...
    public static final class SetFuture<V> implements Runnable {
        final bxl<? extends V> future;
        final AbstractResolvableFuture<V> owner;

        SetFuture(AbstractResolvableFuture<V> abstractResolvableFuture, bxl<? extends V> bxlVar) {
    ...
    */
        ...
        if ((obj == null) || (obj instanceof SetFuture)) {
            ...
            while (true) {
                if (ATOMIC_HELPER.casValue(abstractResolvableFuture, obj, cancellation)) {
                    ...
                    bxl<? extends V> bxlVar = ((SetFuture) obj).future;
                    if (bxlVar instanceof AbstractResolvableFuture) {
                        abstractResolvableFuture = (AbstractResolvableFuture) bxlVar;
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public boolean setFuture(bxl<? extends V> bxlVar) {
        Failure failure;
        checkNotNull(bxlVar);
        Object obj = this.value;
        if (obj == null) {
            if (bxlVar.isDone()) {
                if (!ATOMIC_HELPER.casValue(this, null, getFutureValue(bxlVar))) {
                    return false;
                }
                complete(this);
                return true;
            }
            SetFuture setFuture = new SetFuture(this, bxlVar);
            if (ATOMIC_HELPER.casValue(this, null, setFuture)) {
                try {
                    bxlVar.addListener(setFuture, DirectExecutor.INSTANCE);
                } catch (Throwable th) {
                    try {
        ...
        if (obj instanceof Cancellation) {
            bxlVar.cancel(((Cancellation) obj).wasInterrupted);
        }
        return false;
    }

    static Object getFutureValue(bxl<?> bxlVar) {
        if (bxlVar instanceof AbstractResolvableFuture) {
            Object obj = ((AbstractResolvableFuture) bxlVar).value;
            if (!(obj instanceof Cancellation)) {
                return obj;
            }
        ...
        boolean isCancelled = bxlVar.isCancelled();
        if ((!GENERATE_CANCELLATION_CAUSES) & isCancelled) {
            return Cancellation.CAUSELESS_CANCELLED;
        }
        try {
            Object uninterruptibly = getUninterruptibly(bxlVar);
            return uninterruptibly == null ? NULL : uninterruptibly;
        } catch (CancellationException e) {
            if (!isCancelled) {
                return new Failure(new IllegalArgumentException("get() threw CancellationException, despite reporting isCancelled() == false: ".concat(String.valueOf(bxlVar)), e));
            }
