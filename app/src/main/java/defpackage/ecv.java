...
final class ecv {
    ...
    private static final ecv c = new ecv();
    ...
    public static void a(edx edxVar, edv edvVar) throws IOException {
        int i = 0;
        long j = 0;
        byte b2 = 0;
        while (i < edxVar.g()) {
            int a2 = edxVar.a(i) & 255;
            int i2 = a[a2];
            byte b3 = b[a2];
            j = (j << b3) | i2;
            int i3 = b2 + b3;
        ...
        if (b2 > 0) {
            edvVar.h((int) ((255 >>> b2) | (j << (8 - b2))));
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static int a(edx edxVar) {
        long j = 0;
        for (int i = 0; i < edxVar.g(); i++) {
            j += b[edxVar.a(i) & 255];
        }
        return (int) ((j + 7) >> 3);
    }
